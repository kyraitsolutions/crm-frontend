...
const GlobalSearch = ({ search, openSearch, setOpenSearch }: SearchProps) => {
    ...
    const [searchResult, setSearchResult] = useState<{ leads: any[]; contacts: any[]; }>({
        leads: [],
        contacts: [],
    });
    ...
    const getSearchResult = async () => {
        ...
        try {
            setLoading(true);
            const response = await analyticsService.getSearch(String(accountId), search);
            console.log("response", response.data)
            setSearchResult({
                leads: response?.data?.docs?.leads || [],
                contacts: response?.data?.docs?.contacts || [],
            });
        } catch (error) {
