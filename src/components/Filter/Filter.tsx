...
const Filter = ({
  ...
  const { computedStartDate, computedEndDate } = useMemo(() => {
    if (selectedDays !== undefined) {
      const { startDate, endDate } = getDateRange({ days: selectedDays });
  ...
  }, [selectedDays, localFilters.startDate, localFilters.endDate]);
  ...
  return (
    <div className="absolute inset-0 z-50" onClick={() => setOpenFilter(false)}>
      ...
      >
        ...
        <h3 className="mb-4 text-sm font-semibold text-start">Filters</h3>
          ...
          {(enable?.stage || enable?.read) && (
            <div className="flex gap-2">
              ...
              {enable?.read && (
                <select
                  value={
                    localFilters.read === null ? "" : String(localFilters.read)
                  }
                  onChange={(e) =>
                    updateFilter(
                      "read",
