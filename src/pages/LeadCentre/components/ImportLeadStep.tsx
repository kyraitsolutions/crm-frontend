...
const ImportLeadStep = () => {
    ...
    async function handleMappingComplete(mapping: Record<string, string>) {
        ...
        const payloads = await leadService.buildAllLeadPayloads(String(accountId), sampleRows, mapping);
    ...
    return (
        <div className="pb-10">
            ...
            <div className="flex justify-end items-center gap-3 px-20">
                <button onClick={() => handleStep("sub")} className="rounded-xl border border-gray-300 bg-white px-6 py-2 text-gray-700 hover:bg-gray-50">
                    Cancel
                </button>

                <button onClick={() => handleStep("add")} className="rounded-xl bg-primary px-6 py-2 font-medium text-white">
                    Next
                </button>
            </div>
        </div>
