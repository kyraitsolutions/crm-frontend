...
const EmailEditor = ({
    ...
    return (
        <div className={`absolute ${isMaximize ? "top-0" : "top-16"}  left-0 w-full z-50 bg-black/40 flex items-start justify-end`}>
            <div className={`w-full ${isMaximize ? "h-screen w-screen" : "h-[93vh] max-w-7xl"}  duration-100  bg-white shadow-2xl rounded-t-md overflow-hidden flex flex-col`}>
                {/* Top Header */}
                <div className="h-11 bg-[#5468ff] text-white flex items-center justify-between px-5">
                    ...
                    <div className="flex items-center gap-3">
                        <button>
                            {!isMaximize ? <Maximize2 onClick={() => setIsMaximize(true)} size={18} />
                ...
                {/* CC */}
                {ccOpen && (<div className="py-2 border-b">
                ...
                <div className="border-t px-5 py-4 flex items-center justify-between">
                    <ButtonWithTitle title="Attachment" className=" w-8 h-8 rounded-md text-gray-500 flex items-center justify-center">
                        <Paperclip size={18} />
                    </ButtonWithTitle>
