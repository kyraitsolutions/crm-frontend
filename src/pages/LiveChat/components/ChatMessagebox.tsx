import WhatsAppComposer from "../composer/whatsapp/WhatsAppComposer";

interface ChatMessageboxProps {
  platform?: string;
}

const ChatMessagebox = ({ platform }: ChatMessageboxProps) => {
  switch (platform) {
    case "whatsapp":
      return <WhatsAppComposer />;

    default:
      return null;
  }
};

export default ChatMessagebox;

// <div className="px-5 bg-white h-22 flex items-center relative">
//   <input
//     type="text"
//     placeholder="Type your message here..."
//     className="w-full bg-gray-100 text-sm rounded-2xl py-5 px-3 focus:outline-none focus:ring-1 focus:ring-primary focus:ring-offset-0"
//   />

//   <button className="absolute right-13 bottom-11 rounded-2xl  transform translate-1/2 text-white bg-primary p-3">
//     <MdSend size={20} />
//   </button>
// </div>
