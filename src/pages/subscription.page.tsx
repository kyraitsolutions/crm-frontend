            ...
            <CardContent className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-6">
                {plans?.map((plan) => (
                    <div
                        ...
                        className={`border shadow-sm p-6 flex flex-col justify-between ${plan.highlight ? "border-primary shadow-lg" : ""
                            }`}
                    >
                        <div>
                            <h3 className="text-xl font-semibold">{plan.name === 'payg' ? "pay as you go" : plan.name}</h3>
                        ...
                        <Button
                            ...
                            variant={plan._id === authUser?.usersubscription.planId ? "secondary" : "default"}
                            disabled={plan._id === authUser?.usersubscription.planId}
                        >
                            {plan._id === authUser?.usersubscription.planId ? "Current Plan" : "Choose Plan"}
                        </Button>
                    </div>
                ))}
            </CardContent>
